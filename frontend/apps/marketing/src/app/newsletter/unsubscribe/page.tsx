...
export default async function UnsubscribePage({ searchParams }: { searchParams: Promise<{ t?: string | string[] }> }) {
  ...
  if (token) {
    try {
      valid = await inspectNewsletterToken("unsubscribe", token);
    } catch (error) {
          ...
          <div className="flex max-w-[52rem] flex-col gap-4">
            ...
            <p className={styles.pageDescription}>
              {isError
                ...
                : "You are about to unsubscribe from the Open Agency newsletter."}
            </p>
            {valid && token ? <UnsubscribeForm token={token} /> : null}
